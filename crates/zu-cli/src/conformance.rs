...
const NOTES: &[&str] = &[
    ...
    "four of the standard's conditions are ones no statement reaches here, and none of the \
     four is a gap a case could close. 22G04 says two values are not comparable, and zu \
     supports GA04, so every pair of values has an order and an engine whose x < y said \
     unknown while its ORDER BY x put x first would have two orders and be wrong in one of \
     them. 22G13 says a group variable stood where one value belongs, and zu supports GQ17, \
     so a property read off a group is the list of that property over the group. 25G02 says \
     catalog and data statements do not mix in one transaction, and they do here, under GP18. \
     25G04 says two graphs do not, and they do, under GT03. 08007 and 40003 were counted here \
     as well and are not any more: neither is about a statement, and a client that destroys \
     the connection with a commit or a rollback in flight reaches both, which is the only way \
     either is reached at all",
