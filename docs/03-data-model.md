...
- Declared limits, which exist so that a statement asking for more is told which limit it reached rather than failing generally: a node carries at least the one label its table is named by and at most 64, because a row's label set is one word; an edge carries the one label its rel table is named by, since a statement changing the labels of an edge would be moving it between tables; an element or an edge carries at most 4096 properties. Each of those five has its own status code in ISO 39075 and zu raises the one the standard names, counting a written pattern's own properties before a column is read so the refusal does not depend on the table.
- One value per property per clause: a `SET` assigning to the same property twice, or to an element's whole record and a property of it, is refused rather than picking one. Two `SET` clauses in a row are two clauses and stay last wins, which is what running one after another means.
- An element a `DELETE` in the same statement took away is gone for the clauses after it: reading a property off it and writing an edge onto it are both refused by name. A write folds as it is staged, so the clause after a delete reads a file the tombstone is already in.
