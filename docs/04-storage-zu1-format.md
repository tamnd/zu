...
Design lineage: DuckDB file layout (dual headers, fixed blocks, sidecar WAL, checkpoint-into-columns), Kùzu node-group CSR (slack gaps), Lance 2.1 structural encodings, BtrBlocks sampled cascades. All integers little-endian. CRC32C everywhere. Format id: `zu1`, version 2.
...
Everything above is about a file's contents and none of it is about the filesystem. Two traits keep that true. `VfsFile` is one open file and its methods are exactly the syscalls this format issues, which is positioned reads and writes, a length change and the two sync flavors; `Vfs` is where a file comes from, which is a create that refuses to clobber, a read-write open, a read-only one, and the log's open-or-create. A database carries the `Vfs` it was opened through, because the sidecar log is opened by name off the base's path long after the base was opened, and so is every second handle a query worker forks.

`RealVfs` is the filesystem and is what every open uses unless it was told otherwise. `MemVfs` is a map from name to bytes, which is what a database with no file on disk is made of: the same headers, the same log, the same fold and the same recovery, on bytes that go away with the process. `RecordingFile` is the third, and it is why the layer exists at all: the crash harness logs every mutating call, cuts the log at any syscall boundary, rebuilds the image a crash there could leave, and checks recovery against it (§9). A database in memory is not a mode this format grew; it is the second answer to a question the format was already asking.
...
SegmentMeta {
  ...
  blocks: [BlockPtr], crcs: [u32], // one crc32c per block, contiguous runs preferred
  value_count: u32, null_count: u32,
  zone: { min, max (truncated to 16 B), has_null },
  uncompressed_bytes: u64, live_bytes: u64,
}
```
