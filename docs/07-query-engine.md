...
A query reads one graph, and `USE` is what names it (GQ01, ISO 16.2). A query with no `USE` in front of it is against the graph the session is working in, which is the home graph, and `USE CURRENT_PROPERTY_GRAPH` says that out loud. `USE g` names a graph the catalog holds, by a bare name in the root schema or by a path. ISO writes the clause over a graph expression; a name and the current graph are the two spellings that name a graph without running one, and a graph an expression computes is not a graph a file holds. A name the catalog does not hold is a reference that resolves to nothing, which is `42002`.
