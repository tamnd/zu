...
| TableFunction | `pagerank()`, `wcc()`, `louvain()`, `bfs()`, `sssp()`, `sssp_weighted()`, `cdlp()`, `lcc()`, `triangle_count()` … return relations, composable with MATCH (GraphAlg direction; no Pregel API) |
