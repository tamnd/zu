...
A statement block is the same mixing written without the words around it. `CREATE GRAPH side ANY NEXT INSERT (:thing {n: 1}) NEXT MATCH (t:thing) RETURN COUNT(t) AS n` is one statement, and the savepoint it takes is one savepoint for all three parts: the graph the first part made is there for the parts behind it, and a part that raises takes the catalog change and the rows back together. Each part runs the way it would have run on its own, plan cache and write splitting and all, so what a block adds is the order and the one savepoint around it. That is why `START TRANSACTION` and the two words that end one are refused among the parts: the block is already the unit, and a word saying where a transaction begins would be saying it is two. A block written inside the body of a call the statement begins with is the same block: the catalog parts come out of the body and stand in front of the statement, so what runs is the parts in the order they were written under the one savepoint, and a raise anywhere in them takes the graph the body made back with the rows.
