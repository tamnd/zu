...
- Binary size: no default `regex`/`chrono` heavy deps (own date code); measured in CI (T7 gate: 15 MiB), on what a release ships rather than on what a developer builds. The `dist` profile is that build: `release` with a fat link, and `opt-level = "s"` on arrow, parquet and their codecs. Those crates are 5.12 MiB of a 13.03 MiB CLI on darwin, since the CLI carries the Parquet reader dx/12 section 6 promises and libzu does not, and the two together take it to 10.45 MiB. Nothing zu wrote is asked to be small: the engine, the encodings and the kernels stay at `opt-level = 3`, and the benches keep the thin link so the perf gates read the codegen a developer gets.
- `panic = "abort"` is the size lever that is not taken and cannot be taken as things stand. It is a property of a profile, not of an artifact, and `scripts/libzu-build.sh` builds the CLI and the C ABI in one invocation, so a profile that aborts would abort inside `zu_*` calls that catch a panic today and answer `ZU_STATUS_ERROR`. Taking it would mean two invocations with two profiles, which is a second full link of the same dependency tree per target; that is a worse trade than the megabyte until the ceiling is close again.
