...
What a user reads comes back on a separate `zu_error` handle carrying the GQLSTATUS code, the severity, and the message as fields rather than as one string to parse. Those are what a binding needs as data rather than as prose: the code picks which exception class to raise, the severity decides whether to raise at all, and neither survives being formatted into a sentence and parsed back out. A failure with no condition reports no code rather than a guessed one, because a binding mapping codes to classes has to tell a missing code from one it does not recognize. The status enum stays small on purpose and carries no conditions: there are several hundred of those and no `switch` should ever be written over them. The engine's own error type maps onto the enum in one exhaustive `match` with no wildcard arm, so a new variant fails to compile there rather than reaching nine bindings as whichever status a wildcard happened to name.

Positions are not on the handle yet. The engine renders one into the message where it has one and carries no structured position anywhere, so exposing accessors now would mean either parsing our own prose back apart or freezing a field that is always absent. R9 is additive, so the accessors can land the day the parser carries a position, and the reverse is not true.
